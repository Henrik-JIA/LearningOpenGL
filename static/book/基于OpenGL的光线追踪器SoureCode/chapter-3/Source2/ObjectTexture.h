...
void getTexture(std::vector<Mesh> & data, Shader& shader, ObjectTexture& objTex) {
	...
	float *vertexArray = new float[(3 + 3 + 2) * dataSize_v];
	float *faceArray = new float[(1) * dataSize_f];
	int v_index = 0;
	int f_index = 0;
	for (int i = 0; i < data.size(); i++) {
		// 处理每个Mesh的顶点
		for (int j = 0;j < data[i].vertices.size(); j++) {
			vertexArray[v_index * (8) + 0] = data[i].vertices[j].Position.x;
			vertexArray[v_index * (8) + 1] = data[i].vertices[j].Position.y;
			vertexArray[v_index * (8) + 2] = data[i].vertices[j].Position.z;
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, 4000 * 8, 1, 0, GL_RED, GL_FLOAT, vertexArray);
	// 最近邻插值
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, 4000, 1, 0, GL_RED, GL_FLOAT, faceArray);
	// 最近邻插值
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
