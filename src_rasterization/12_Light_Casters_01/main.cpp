...
void main() {
  ...
  // 光源距离
  float lightDistance = length(light.position - outFragPos);
  ...
  // vec3 lightDir = normalize(light.position - outFragPos); // 使用点光源
  vec3 lightDir = normalize(light.parallelLightDirection); // 使用平行光
  float diff = max(dot(norm, lightDir), 0.0);
  // vec3 diffuse = (light.diffuseStrength/(lightDistance * lightDistance)) * diff * diffuseTextureColor;
  vec3 diffuse = light.diffuseStrength * diff * diffuseTextureColor; // 平行光的光强不会随距离衰减
  ...
  // vec3 specular = (light.specularStrength/(lightDistance * lightDistance)) * spec * specularTextureColor;
  vec3 specular = light.specularStrength * spec * specularTextureColor; // 平行光的光强不会随距离衰减
