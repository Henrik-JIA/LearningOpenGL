...
void main()
{
    ...
    // gl_Position = projection * view * model * vec4(aPos + offset, 1.0); // 直接相加
    vec3 pos = aPos * (gl_InstanceID / 100.0); // 获取实例id，以id的百分比对位置进行缩放
    gl_Position = projection * view * model * vec4(pos + aOffset, 1.0); // 直接相加
    fColor = aColor;
}
)";
...
int main()
{
  ...
  glGenVertexArrays(1, &quadVAO); // 生成VAO
  glGenBuffers(1, &quadVBO); // 生成VBO
  glGenBuffers(1, &instanceVBO); // 生成实例VBO

  glBindVertexArray(quadVAO); // 绑定VAO

  glBindBuffer(GL_ARRAY_BUFFER, quadVBO); // 绑定VBO
  glBufferData(GL_ARRAY_BUFFER, sizeof(quadVertices), quadVertices, GL_STATIC_DRAW); // 设置VBO数据
  
  glEnableVertexAttribArray(0); // 启用位置属性
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)0); // 读取前3个float作为位置
  glEnableVertexAttribArray(1); // 启用颜色属性
  glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)(3 * sizeof(float))); // 后3个float作为颜色
  
  glBindBuffer(GL_ARRAY_BUFFER, 0); // 解绑VBO

  glBindBuffer(GL_ARRAY_BUFFER, instanceVBO); // 绑定实例VBO
  glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * 100, &translations[0], GL_STATIC_DRAW); // 设置实例VBO数据
  
  glEnableVertexAttribArray(2); // 启用实例位置属性
  glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0); // 读取前3个float作为位置
  glVertexAttribDivisor(2, 1); // 对location=2的属性设置逐实例更新，第一个参数为属性id，第二个参数表示每一个实例更新一次属性，如果第二个参数为2，则每两个实例更新一次属性。
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    // 实例化着色器
    glBindVertexArray(quadVAO); // 绑定VAO
    instanceShader.use(); // 激活实例化着色器
    glDrawArraysInstanced(GL_TRIANGLES, 0, 6, 100); // 绘制100个实例，每个实例6个顶点，之所以是6个顶点，是因为没有使用EBO元素索引。
