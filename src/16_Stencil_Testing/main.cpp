...
// 相机系统
Camera camera(glm::vec3(0.0f, 1.0f, 6.0f)); // 相机类
...
int main()
{
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    // ************************************************************
    // 1.正常绘制对象写入模板缓冲区
    glStencilFunc(GL_ALWAYS, 1, 0xff); // 填充模板
    glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE); // 测试通过后的行为，用新值替换旧值，将1写入模板缓冲区。（默认情况应该都是0）
    ...
    // ************************************************************
    // 2.绘制盒子放大版本，然后禁用模板写入，盒子轮廓
    glStencilFunc(GL_NOTEQUAL, 1, 0xff); // 读取模板，如果模板值不等于1，则通过测试
    glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP); // 测试通过后的行为，不进行任何操作
    ...
    for(unsigned int i = 0; i < sizeof(cubePositions)/sizeof(cubePositions[0]); i++){
        ...
        glm::mat4 model = glm::mat4(1.0f);
        model = glm::translate(model, cubePositions[i]);
        model = glm::scale(model, cubeScales[i] * scale); // 应用原始缩放比例
